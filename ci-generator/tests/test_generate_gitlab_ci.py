...
class TestGitlabCiGenerator(TestCase):
    ...
    def test_get_dockerfile_name_to_use(self):
        ...
        dockerfile = ci_generator.get_dockerfile_name_to_use("linux-il2cpp", {})
        self.assertEqual("unitysetup-linux-il2cpp", dockerfile)
    ...
    def test_set_dockerfile_to_unity_versions(self):
        with self.subTest("empty unity_versions stays empty"):
            ...
            self.do_test_set_dockerfile_to_unity_versions(
                expected_unity_versions, unity_versions
            )
        ...
        with self.subTest(
            "android component in unity_versions uses different dockerfile"
        ):
            unity_versions = {
                "2019.1.3f1": {"platforms": {"android": {"components": "unity"}}}
            }
            expected_unity_versions = {
                "2019.1.3f1": {
                    "platforms": {
    ...
    def do_test_set_dockerfile_to_unity_versions(
        self, expected_unity_versions, unity_versions
    ):
        ci_generator = GitlabCiGenerator()
        new_unity_versions = ci_generator.set_dockerfile_to_unity_versions(
            unity_versions
        )
    ...
    def test_output(self):
        ci_generator = GitlabCiGenerator()
        with mock.patch(
            f"{self.class_path}.get_rendered_ci_template"
        ) as mocked_get_rendered_ci_template:
    ...
    def do_test_snapshot(self, ci_generator, version_input, version_output):
        ...
        with captured_output() as (out, err):
            with mock.patch(
                f"{self.class_path}.get_unity_versions_path"
